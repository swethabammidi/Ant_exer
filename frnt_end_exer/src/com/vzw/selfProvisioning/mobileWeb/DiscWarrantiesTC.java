 ...
 */
public class DiscWarrantiesTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(DiscWarrantiesTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Disc Warranties TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("DiscWarrantiesTC.doAction():" + logTag +
			"User has entered the Disc Warranties TC page, with the following session: "+get_session().getId());
		return "mobileWeb/discWarrantiesTC.jsp";
	}
