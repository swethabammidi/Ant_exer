 ...
 */
public class CopyrightTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(CopyrightTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Copyright TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("CopyrightTC.doAction():" + logTag +
			"User has entered the Copyright TC page, with the following session: "+get_session().getId());
		return "mobileWeb/copyrightTC.jsp";
	}
