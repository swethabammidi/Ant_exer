 ...
 */
public class EntireTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(EntireTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		int service = Util.getService(req);
		L.info("doAction():" + logTag +
			"User has entered the Entire TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("EntireTC.doAction():" + logTag +
			"User has entered the Entire TC page, with the following session: "+get_session().getId());
		if (service == Util.PPUS)
		    return "mobileWeb/entireTCPPUS.jsp";
		else
		    return "mobileWeb/entireTC.jsp";
	}
