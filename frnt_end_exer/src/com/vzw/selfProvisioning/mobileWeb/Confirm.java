 ...
 */
public class Confirm extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(Confirm.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the confirm page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("Confirm.doAction():" + logTag +
			"User has entered the confirm page, with the following session: "+get_session().getId());
		
		return "mobileWeb/confirm.jsp";
	}
