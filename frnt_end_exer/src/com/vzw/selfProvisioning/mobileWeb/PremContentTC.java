 ...
 */
public class PremContentTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(PremContentTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Prem Content TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("PremContentTC.doAction():" + logTag +
			"User has entered the Prem Content TC page, with the following session: "+get_session().getId());
		return "mobileWeb/premContentTC.jsp";
	}
