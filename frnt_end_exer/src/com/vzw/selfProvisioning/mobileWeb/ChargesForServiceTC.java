 ...
 */
public class ChargesForServiceTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(ChargesForServiceTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Charges For Service TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("ChargesForServiceTC.doAction():" + logTag +
			"User has entered the Charges For Service TC page, with the following session: "+get_session().getId());
		return "mobileWeb/chargesForServiceTC.jsp";
	}
