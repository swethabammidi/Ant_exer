 ...
 */
public class MonConsentTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(MonConsentTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Mon Consent TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("MonConsentTC.doAction():" + logTag +
			"User has entered the Mon Consent TC page, with the following session: "+get_session().getId());
		return "mobileWeb/monConsentTC.jsp";
	}
