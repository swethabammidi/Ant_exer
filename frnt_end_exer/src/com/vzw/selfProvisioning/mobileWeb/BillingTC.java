 ...
 */
public class BillingTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(BillingTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Billing TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("BillingTC.doAction():" + logTag +
			"User has entered the Billing TC page, with the following session: "+get_session().getId());
		return "mobileWeb/billingTC.jsp";
	}
