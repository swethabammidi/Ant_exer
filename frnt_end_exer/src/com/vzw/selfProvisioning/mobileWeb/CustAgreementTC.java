 ...
 */
public class CustAgreementTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(CustAgreementTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Customer Agreement TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("CustAgreementTC.doAction():" + logTag +
			"User has entered the Customer Agreement TC page, with the following session: "+get_session().getId());
		return "mobileWeb/custAgreementTC.jsp";
	}
