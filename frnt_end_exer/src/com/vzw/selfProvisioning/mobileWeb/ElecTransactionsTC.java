 ...
 */
public class ElecTransactionsTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(ElecTransactionsTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Elec Transactions TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("ElecTransactionsTC.doAction():" + logTag +
			"User has entered the Elec Transactions TC page, with the following session: "+get_session().getId());
		return "mobileWeb/elecTransactionsTC.jsp";
	}
