 ...
 */
public class LimLiabilitiesTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(LimLiabilitiesTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Lim Liabilities TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("LimLiabilitiesTC.doAction():" + logTag +
			"User has entered the Lim Liabilities TC page, with the following session: "+get_session().getId());
		return "mobileWeb/limLiabilitiesTC.jsp";
	}
