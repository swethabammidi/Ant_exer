 ...
 */
public class ConfidentialityTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(ConfidentialityTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Confidentiality TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("ConfidentialityTC.doAction():" + logTag +
			"User has entered the Confidentiality TC page, with the following session: "+get_session().getId());
		return "mobileWeb/confidentialityTC.jsp";
	}
