 ...
 */
public class VcastMoreInfo extends RequestResponse implements ActionInterface {

	private static Logger L = Logger.getLogger(SPProps.getFELogName(VcastMoreInfo.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the VCAST More Info page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("VcastTC.doAction():" + logTag +
			"User has entered the VCAST More Info page, with the following session: "+get_session().getId());

		
		return "mobileWeb/VODInfo.jsp";
