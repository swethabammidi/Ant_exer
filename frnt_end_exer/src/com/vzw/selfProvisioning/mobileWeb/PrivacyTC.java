 ...
 */
public class PrivacyTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(PrivacyTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Privacy TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("PrivacyTC.doAction():" + logTag +
			"User has entered the Privacy TC page, with the following session: "+get_session().getId());
		return "mobileWeb/privacyTC.jsp";
	}
