 ...
 */
public class UseOfServicesTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(UseOfServicesTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Use Of Services TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("UseOfServicesTC.doAction():" + logTag +
			"User has entered the Use Of Services TC page, with the following session: "+get_session().getId());
		return "mobileWeb/useOfServicesTC.jsp";
	}
