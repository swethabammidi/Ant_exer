 ...
 */
public class Subscribe extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(Subscribe.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the price info page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("Subscribe.doAction():" + logTag +
			"User has entered the price info page, with the following session: "+get_session().getId());
		
		return "mobileWeb/priceInfo.jsp";
	}
