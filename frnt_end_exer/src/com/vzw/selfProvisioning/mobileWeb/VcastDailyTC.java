 ...
 */
public class VcastDailyTC extends RequestResponse implements ActionInterface {

	private static Logger L = Logger.getLogger(SPProps.getFELogName(VcastDailyTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the VCAST daily T&C page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("VcastTC.doAction():" + logTag +
			"User has entered the VCAST daily T&C page, with the following session: "+get_session().getId());

		
		return "mobileWeb/dailyVODTandC.jsp";
