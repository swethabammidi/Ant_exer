 ...
 */
public class MobileWebDesc extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(MobileWebDesc.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the More Info 1 page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("MobileWebDesc.doAction():" + logTag +
			"User has entered the More Info 1 page, with the following session: "+get_session().getId());
		
		return "mobileWeb/moreInfo1.jsp";
	}
