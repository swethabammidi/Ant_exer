 ...
 */
public class Cancel extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(Cancel.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info(	"doAction():" + logTag + 
			"User has entered the cancel page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("Cancel.doAction():" + logTag +
			"User has entered the Cancel page, with the following session: "+get_session().getId());

		return "mobileWeb/cancel.jsp";
	}
