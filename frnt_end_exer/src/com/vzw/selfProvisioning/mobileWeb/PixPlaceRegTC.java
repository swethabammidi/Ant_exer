 ...
 */
public class PixPlaceRegTC extends RequestResponse implements ActionInterface {
	private static Logger L = Logger.getLogger(SPProps.getFELogName(PixPlaceRegTC.class));
	...
	public String doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		L.info("doAction():" + logTag +
			"User has entered the Pix Place Reg TC page, with the following session: "+get_session().getId());
		SPProps.infoLogger.info("ChargesForServiceTC.doAction():" + logTag +
			"User has entered the Pix Place Reg TC page, with the following session: "+get_session().getId());
		return "mobileWeb/pixPlaceRegTC.jsp";
	}
